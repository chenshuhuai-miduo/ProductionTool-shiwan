package com.miduo.cloud.variant.zhimeizhai.application.code;
...
import com.miduo.cloud.application.code.CodeBloomFilterManager;
...
public class CodeApplicationService {
    ...
    public synchronized ApiResult<Boolean> deleteByBoxCode(String boxCode) {
        try {
            ...
            // 2. 逻辑删除该记录
            int deleteCount = codeRelationMapper.update(null,
            ...
            Long remainingCount = codeRelationMapper.selectCount(
                new LambdaQueryWrapper<CodeRelationPO>()
                    .eq(CodeRelationPO::getOrderNo, orderNo)
            ...
            int updateQtyCount = codeRelationMapper.update(null,
                new LambdaUpdateWrapper<CodeRelationPO>()
                    .eq(CodeRelationPO::getOrderNo, orderNo)
                    .eq(CodeRelationPO::getProductNo, productNo)
                    .eq(CodeRelationPO::getTagNo, tagNo)
    ...
    public ApiResult<CurrentPalletInfoVO> getCurrentPalletInfoByTaskId(Integer taskId) {
        try {
            ...
            try {
                LambdaQueryWrapper<com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO> queryWrapper = 
                    new LambdaQueryWrapper<com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO>()
                        .eq(com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO::getOrderNo, orderNo)
                        .eq(com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO::getProductNo, productNo)
                        .eq(com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO::getTagNo, tagNo)
                        .eq(com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO::getIsDel, 0);
                
                // 无箱码模式需要额外过滤Type=2
                if (type != null && type == 2) {
                    queryWrapper.eq(com.miduo.cloud.variant.zhimeizhai.persistence.mybatis.po.CodeRelationPO::getType, 2);
                }
                
                Long uploadCount = codeRelationMapper.selectCount(queryWrapper);
