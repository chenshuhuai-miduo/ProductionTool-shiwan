...
public class ShiwanM2StatsController implements Initializable {
    ...
    private void onPalletCardClick(MouseEvent e) {
        ...
        try {
            ...
            Window palletOwner = palletCard.getScene().getWindow();
            stage.initOwner(palletOwner);
            stage.initModality(Modality.APPLICATION_MODAL);
            Region palletRoot = (Region) root;
            palletRoot.setMinWidth(800);
            palletRoot.setMinHeight(650);
            FxModalOverlayUtil.applyOverlayScene(stage, palletRoot, palletOwner, new Insets(20));
            stage.showAndWait();
        } catch (Exception ex) {
    ...
    private void onRejectCardClick(MouseEvent e) {
        ...
        try {
            ...
            controller.setContext(getDateText(startDate), getDateText(endDate), safe(orderNoField.getText()));
            Stage stage = new Stage();
            Window rejectOwner = rejectCard.getScene().getWindow();
            stage.initOwner(rejectOwner);
            stage.initModality(Modality.APPLICATION_MODAL);
            Region rejectRoot = (Region) root;
            rejectRoot.setMinWidth(1200);
            rejectRoot.setMinHeight(700);
            rejectRoot.setPrefWidth(1200);
            rejectRoot.setPrefHeight(700);
            FxModalOverlayUtil.applyOverlayScene(stage, rejectRoot, rejectOwner, new Insets(20));
            stage.showAndWait();
        } catch (Exception ex) {
